...
public class GamerOp extends OpMode {
    ...
    public void gripBlock() {
        if (gamepad2.a  || (useOneGamepad && gamepad1.a)) {
            ...
            leftClaw.setPosition(.8); //good
            rightClaw.setPosition(.05); //good
        }
        else if (gamepad2.x || (useOneGamepad && gamepad1.x)) {
            leftClaw.setPosition(0.575); //move in just a bit more
            rightClaw.setPosition(.55); //move in a lot
        }
    ...
    public void dropCap() {
        if(gamepad2.right_trigger != 0 || (useOneGamepad && gamepad1.right_trigger != 0))
            capStick.setPosition(1);
        else if (gamepad2.left_trigger != 0 || (useOneGamepad && gamepad1.left_trigger != 0))
